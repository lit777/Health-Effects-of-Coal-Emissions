...
s.m10<-rnorm(n1+n0, mean = s.covariate%*%beta1_01[index0,], sd = sqrt(s1_0[index0]))
#s.m10[(1+n1):(n1+n0)] <- m0
s.m11<-rnorm(n1+n0, mean = s.covariate%*%beta1_11[index1,], sd = sqrt(s1_1[index1]))
#s.m11[(1):(n1)] <- m1


M1.dist <- qnorm(pmin(1-0.1^15,pmax(pnorm(s.m11, mean = s.covariate%*%beta1_11[index1,], sd = sqrt(s1_1[index1])), 0.1^15)))
M0.dist <- qnorm(pmin(1-0.1^15,pmax(pnorm(s.m10, mean = s.covariate%*%beta1_01[index1,], sd = sqrt(s1_0[index1])), 0.1^15)))
...
s.y1 <- qnorm(pmin(1-0.1^15, pmax(0.1^15, con.joint )),mean = s.covariate%*%gamma11[index1,], sd = sqrt(w1_1[index1]) )
...
s.y0 <- qnorm(pmin(1-0.1^15, pmax(0.1^15, con.joint )),mean = s.covariate%*%gamma01[index1,], sd = sqrt(w1_0[index1]) )

y.sample <- cbind(s.y1,s.y0,s.m10,  s.m11)

#----- Threshold
c <- 0.5

#----- Define Strata
abs1 <- which(abs(y.sample[,3]-y.sample[,4]) <= c)
neg1 <- which((y.sample[,3]-y.sample[,4]) > c)

#----- Estimating Principal Causal Effects
Pr1 <- length(abs1)
Pr2 <- length(neg1)
mean.y1 <- mean(y.sample[abs1,1]); mean.y0 <- mean(y.sample[abs1,2]);
EDE <- mean.y1-mean.y0

mean.y1 <- mean(y.sample[neg1,1]); mean.y0 <- mean(y.sample[neg1,2]);
EAE <- mean.y1-mean.y0

result<-c(EDE, EAE, Pr1, Pr2)
